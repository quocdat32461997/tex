...
class Form1040Agent(BaseAgent):
    ...
    def __init__(
        ...
        workflow.add_edge(
            START,
            "process_f1040",  # "check_missing_statements"
        )
        ...
        # workflow.add_node(
        #     "request_statements_with_instruction",
        #     ReActRegistry.get("request_statements_with_instruction"),
        # )
        ...
        for line in lines[:10]:
            # Request any missing statement
            workflow.add_node(
                f"request_statement_{line["name"]}",
                ReActRegistry.get(
                    "request_statements_with_instruction",
                    line=line["context"],
                ),
            )
            workflow.add_edge(prev_line, f"request_statement_{line["name"]}")
            prev_line = f"request_statement_{line["name"]}"

            # Extract statement
            workflow.add_node(
                f"extract_statement_{line["name"]}",
                ReActRegistry.get("extract_statement"),
            )
            workflow.add_edge(prev_line, f"extract_statement_{line["name"]}")
            prev_line = f"extract_statement_{line["name"]}"
